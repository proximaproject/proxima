...
*Setup instructions for a Gitian build of Proxima Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Proxima
...
- [Building Proxima Core](#building-proxima-core)
...
Clone the git repositories for proxima and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/proximaproject/proxima --recursive
...
```bash
sudo cp /home/debian/proxima/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the proxima repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Proxima Core
----------------

To build Proxima Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the proxima repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/proxima/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/proximaproject/proxima
...
For example:
```bash
URL=https://github.com/laanwj/proxima.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit proxima=${COMMIT} --url proxima=${URL} ../proxima/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit proxima=${COMMIT} --url proxima=${URL} ../proxima/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit proxima=${COMMIT} --url proxima=${URL} ../proxima/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-proxima:
./bin/gbuild --commit proxima=${COMMIT},cpp-eth-proxima=${ETHCOMMIT} --url proxima=${URL},cpp-eth-proxima=${ETHURL} ../proxima/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the proxima git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../proxima/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/proxima
SIGPATH=/some/root/path/proxima-detached-sigs

./bin/gbuild --url proxima=${BTCPATH},signature=${SIGPATH} ../proxima/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/proxima-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/proxima-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/proxima-osx-build.assert
```
...
[proximaproject/gitian.sigs](https://github.com/proximaproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@proxima.org) and he will commit them.
