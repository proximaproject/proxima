...
void checkResult(bool isCreation, std::vector<ProximaTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    ProximaTxConverter converter(transaction, NULL);
    ExtractProximaTX proximaTx;
    BOOST_CHECK(converter.extractionProximaTransactions(proximaTx));
    std::vector<ProximaTransaction> result = proximaTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(proximatxconverter_tests, TestingSetup)
