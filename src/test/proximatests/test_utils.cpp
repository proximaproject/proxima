...
void initState(){
    ...
    const std::string dirProxima = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<ProximaState>(new ProximaState(dev::u256(0), ProximaState::openDB(dirProxima, hashDB, dev::WithExisting::Trust), dirProxima + "/proximaDB", dev::eth::BaseState::Empty));
...
ProximaTransaction createProximaTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    ProximaTransaction txEth;
    if(recipient == dev::Address()){
        txEth = ProximaTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = ProximaTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<ProximaTransaction> txs){
