...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/proximaproject/proxima>";
    const std::string URL_WEBSITE = "<http://proxima.im>";
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                dev::eth::Ethash::init();
                boost::filesystem::path proximaStateDir = GetDataDir() / "stateProxima";
                bool fStatus = boost::filesystem::exists(proximaStateDir);
                const std::string dirProxima(proximaStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsPROXtate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<ProximaState>(new ProximaState(dev::u256(0), ProximaState::openDB(dirProxima, hashDB, dev::WithExisting::Trust), dirProxima, existsPROXtate));
