...
#if defined(NDEBUG)
# error "Proxima cannot be compiled without assertions."
#endif
...
std::unique_ptr<ProximaState> globalState;
...
const std::string strMessageMagic = "Proxima Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // proxima
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            ProximaDGP proximaDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = proximaDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = proximaDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            ProximaTxConverter converter(tx, NULL);
            ExtractProximaTX resultConverter;
            if(!converter.extractionProximaTransactions(resultConverter)){
            ...
            std::vector<ProximaTransaction> proximaTransactions = resultConverter.first;
            std::vector<EthTransactionParams> proximaETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(ProximaTransaction proximaTransaction : proximaTransactions){
                sumGas += proximaTransaction.gas() * proximaTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, proximaTransaction.gasPrice());
                } else {
                    txMinGasPrice = proximaTransaction.gasPrice();
                }
                VersionVM v = proximaTransaction.getVersion();
                if(v.format!=0)
                ...
                if(proximaTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(proximaTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += proximaTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)proximaTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(proximaETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > proximaTransactions.size())
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s PROX additional fees, %d delta bytes\n",
...
static DisconnectResult DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // proxima
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // proxima

    if(pfClean == NULL && fLogEvents){
        boost::filesystem::path stateDir = GetDataDir() / "stateProxima";
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    ProximaTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<ProximaTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path proximaDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(proximaDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(proximaDir.string(), std::ios::in | std::ios::out);
...
bool ProximaTxConverter::extractionProximaTransactions(ExtractProximaTX& proximatx){
    std::vector<ProximaTransaction> resultTX;
    ...
    proximatx = std::make_pair(resultTX, resultETP);
    return true;
}

bool ProximaTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool ProximaTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
ProximaTransaction ProximaTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    ProximaTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = ProximaTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = ProximaTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // proxima
    ProximaDGP proximaDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setProximaSchedule(proximaDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = proximaDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = proximaDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = proximaDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // proxima
...
///////////////////////////////////////////////////////////////////////////////////////// proxima
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            ProximaTxConverter convert(tx, &view, &block.vtx);

            ExtractProximaTX resultConvertProximaTX;
            if(!convert.extractionProximaTransactions(resultConvertProximaTX)){
            ...
            if(!CheckMinGasPrice(resultConvertProximaTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertProximaTX.first, blockGasLimit);
            ...
            for(ProximaTransaction& qtx : resultConvertProximaTX.first){
            ...
            if (fLogEvents)
            {
                for(size_t k = 0; k < resultConvertProximaTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertProximaTX.first[k].from(), resultConvertProximaTX.first[k].to(),
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // proxima
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // proxima
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // proxima
            globalState->setRootUTXO(oldHashUTXORoot); // proxima
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // proxima
        uint32_t sizeBlockDGP = proximaDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Proxima PoS we need to sync this after every block to ensure txdb is populated for
