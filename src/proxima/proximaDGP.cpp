#include "proximaDGP.h"

void ProximaDGP::initDataEIP158(){
...
bool ProximaDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule ProximaDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t ProximaDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t ProximaDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t ProximaDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t ProximaDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool ProximaDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void ProximaDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void ProximaDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void ProximaDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void ProximaDGP::createParamsInstance(){
...
dev::Address ProximaDGP::getAddressForBlock(unsigned int blockHeight){
...
void ProximaDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void ProximaDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void ProximaDGP::parseStorageOneUint64(uint64_t& value){
...
void ProximaDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule ProximaDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
