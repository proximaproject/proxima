...
#include "proximastate.h"

using namespace std;
using namespace dev;
using namespace dev::eth;

ProximaState::ProximaState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = ProximaState::openDB(_path + "/proximaDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ProximaState::ProximaState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute ProximaState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, ProximaTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createProximaAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            proxima::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> ProximaState::vins() const // temp
{
...
void ProximaState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* ProximaState::vin(dev::Address const& _a) const
{
    return const_cast<ProximaState*>(this)->vin(_a);
}

Vin* ProximaState::vin(dev::Address const& _addr)
{
...
// void ProximaState::commit(CommitBehaviour _commitBehaviour)
// {
...
void ProximaState::kill(dev::Address _addr)
{
...
void ProximaState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address ProximaState::createProximaAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void ProximaState::deleteAccounts(std::set<dev::Address>& addrs){
...
void ProximaState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void ProximaState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
