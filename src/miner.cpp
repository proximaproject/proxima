...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setProximaSchedule(proximaDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = proximaDGP.getBlockSize(nHeight);
    minGasPrice = proximaDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = proximaDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    ProximaTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractProximaTX resultConverter;
    if(!convert.extractionProximaTransactions(resultConverter)){
    ...
    for(ProximaTransaction proximaTransaction : proximaTransactions){
        txGas += proximaTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + proximaTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(proximaTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, proximaTransactions, hardBlockGasLimit);
...
void ThreadStakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("proximacoin-miner");
...
void StakePROX(bool fStake, CWallet *pwallet)
{
