...
bool WalletModel::isUnspentAddress(const std::string &proximaAddress) const
{
    LOCK2(cs_main, wallet->cs_wallet);
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        if(fValidAddress && CBitcoinAddress(address).ToString() == proximaAddress && out.tx->tx->vout[out.i].nValue)
        {
            return true;
        }
    }
