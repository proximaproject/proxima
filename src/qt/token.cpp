...
bool ToHash160(const std::string& strProximaAddress, std::string& strHash160)
{
    CBitcoinAddress proximaAddress(strProximaAddress);
    if(proximaAddress.IsValid()){
        CKeyID keyid;
        proximaAddress.GetKeyID(keyid);
...
bool ToProximaAddress(const std::string& strHash160, std::string& strProximaAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CBitcoinAddress proximaAddress;
    proximaAddress.Set(keyid);
    if(proximaAddress.IsValid()){
        strProximaAddress = proximaAddress.ToString();
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToProximaAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToProximaAddress(tokenEvent.receiver, tokenEvent.receiver);
